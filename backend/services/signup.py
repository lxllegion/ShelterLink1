...
def create_donor(donor: Donor):
    ...
    - Inserts donor record into the database
    - Returns the saved donor info
    """
    try:
...
def create_shelter(shelter: Shelter):
    ...
    - Validates unique shelter name and email
    - Inserts shelter record into the database
    - Returns the saved shelter info
    """
    try:
        with engine.connect() as conn:
            trans = conn.begin()
